...
// Глобальное хранилище для извлеченного текста документов
export const extractedTextMap = new Map();

// Очистка старых текстов каждые 6 часов
setInterval(() => {
    const now = Date.now();
    for (const [key, value] of extractedTextMap.entries()) {
        if (now - value.timestamp > 6 * 3600 * 1000) {
            extractedTextMap.delete(key);
        }
    }
}, 3600000);
...
router.post('/chat/completions', async (req, res) => {
    try {
        ...
        let files = lastUserMessage.files || []; // ← ИЗВЛЕКАЕМ FILES
        
        // --- TEXT INJECTION LOGIC ---
        // Ищем документы, текст которых был извлечен, чтобы добавить их в prompt
        const remainingFiles = [];
        let appendedText = '';
        
        for (const fileObj of files) {
            // fileObj может иметь file_id или id
            const fId = fileObj.file_id || fileObj.id;
            if (fId && extractedTextMap.has(fId)) {
                const docData = extractedTextMap.get(fId);
                appendedText += `\n\n--- [Содержимое документа: ${docData.name}] ---\n${docData.text}\n--- [Конец документа] ---\n`;
                // НЕ добавляем этот файл в remainingFiles, чтобы Qwen не пытался его парсить как картинку
                logInfo(`Текст документа ${docData.name} успешно внедрен в запрос`);
            } else {
                // Это картинка или неизвестный файл, оставляем для Qwen Vision API
                remainingFiles.push(fileObj);
            }
        }
        ...
        if (appendedText) {
            if (Array.isArray(messageContent)) {
                messageContent.push({ type: 'text', text: appendedText });
            } else if (typeof messageContent === 'string') {
                messageContent += appendedText;
            } else {
                messageContent = appendedText;
            }
        }
    ...
    try {
        ...
        try {
            if (mimeType.includes('pdf') || originalName.endsWith('.pdf')) {
                ...
                logInfo(`Извлечен текст из PDF: ${extractedText.length} символов`);
            } else if (mimeType.includes('wordprocessingml') || originalName.endsWith('.docx')) {
                const result = await mammoth.extractRawText({ path: req.file.path });
                extractedText = result.value;
                logInfo(`Извлечен текст из DOCX: ${extractedText.length} символов`);
                ...
                extractedText = text;
                logInfo(`Извлечен текст из XLSX: ${extractedText.length} символов`);
            } else if (mimeType.includes('presentationml') || originalName.endsWith('.pptx') || originalName.endsWith('.ppt')) {
                const parsed = await officeParser.parseOffice(req.file.path);
                extractedText = typeof parsed === 'string' ? parsed : (parsed && parsed.toText ? parsed.toText() : JSON.stringify(parsed));
                logInfo(`Извлечен текст из PPTX: ${extractedText ? extractedText.length : 0} символов`);
            } else if (mimeType.includes('text/plain') || originalName.endsWith('.txt') || originalName.endsWith('.md')) {
                extractedText = fs.readFileSync(req.file.path, 'utf8');
                logInfo(`Извлечен текст из TXT: ${extractedText.length} символов`);
            }
        } catch (e) {
            logError(`Ошибка при извлечении текста из ${originalName}: ${e.message}`);
        }

        // Загрузка в OSS (для единообразия, хотя Qwen не сможет прочитать документ, 
        // мы передадим текст напрямую, а OSS загрузка нужна для корректного ответа клиенту)
        ...
        if (result.success) {
            ...
            if (extractedText) {
                // Сохраняем текст в памяти по file_id
                extractedTextMap.set(result.fileId, {
                    text: extractedText,
            ...
            res.json({ success: true, file: { name: result.fileName, url: result.url, size: req.file.size, type: req.file.mimetype, file_id: result.fileId, file_path: result.filePath } });
        } else {
